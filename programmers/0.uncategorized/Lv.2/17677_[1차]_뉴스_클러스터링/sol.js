...
function solution(str1, str2) {
    ...
    // 4. mapA와 mapB의 값으로 해당 원소에 대한 갯수 정해서 교집합,합집합 생성
    const intersection = [];
    const union = [];
    for(let item of mapA.keys()){
        if(mapB.has(item)){
            ...
            for(let i = 1 ; i <= min ; i++) intersection.push(item);
        }
        // A의 여집합,교집합 부분 합집합에 추가
        const max = Math.max(mapA.get(item), mapB.get(item)??0);
        for(let i = 1 ; i <= max ; i++) union.push(item);
    }
    // 4-1. B의 여집합 부분 합집합에 추가
    for(let item of mapB.keys()){
        if(!mapA.has(item)){
            for(let i = 1 ; i <= mapB.get(item) ; i++) union.push(item);
        }
    }
    // 5. 반환
...
// 테스트 4 〉	통과 (1.59ms, 33.7MB)
// 테스트 5 〉	통과 (0.33ms, 33.5MB)
// 테스트 6 〉	통과 (0.37ms, 33.4MB)
// 테스트 7 〉	통과 (0.76ms, 33.6MB)
// 테스트 8 〉	통과 (0.33ms, 33.4MB)
// 테스트 9 〉	통과 (0.72ms, 33.6MB)
// 테스트 10 〉	통과 (0.71ms, 33.5MB)
// 테스트 11 〉	통과 (0.98ms, 33.5MB)
