...
// balt의 내용물을 사용하지는 않으므로 stack 자료구조 대신 숫자를 사용하여 성능 개선 가능

function solution(order) {
    const balt = [...order].sort((a, b)=>b - a); // pop 사용 위해 내림차순 정렬
    const stack = []; // 보조 컨테이너 벨트
    let result = 0;
    
    for(let curOrder of order){
        // curOrder와 같아질 때까지 계속 빼서 stack으로 옮기기
        while(balt.at(-1) < curOrder){
            stack.push(balt.pop())
        }
        // 같아지면 트럭에 넣고 다음 순서의 아이템으로 이동
        if(balt.at(-1) === curOrder){
            balt.pop();
            result += 1;
            continue;
        }
        // stack을 찾아봐야 하는 상황일 때,
        if( balt.length===0 || balt.at(-1) > curOrder){
            // (stack에서 가져올 수 있는 경우)
...
// 정확성  테스트
// 테스트 1 〉	통과 (72.85ms, 42.2MB)
// 테스트 2 〉	통과 (289.25ms, 57.4MB)
// 테스트 3 〉	통과 (280.06ms, 76.6MB)
// 테스트 4 〉	통과 (227.56ms, 68.2MB)
// 테스트 5 〉	통과 (480.71ms, 101MB)
// 테스트 6 〉	통과 (111.11ms, 48MB)
// 테스트 7 〉	통과 (289.88ms, 94.9MB)
// 테스트 8 〉	통과 (19.37ms, 39.5MB)
// 테스트 9 〉	통과 (194.22ms, 81.2MB)
// 테스트 10 〉	통과 (346.16ms, 103MB)